...
const Shopsecondsection = () => {
      ...
      const getdata = async() => {
        const res = await axios.get(`http://127.0.0.1:8000/api/products/fetched?search=${search}`)
      ...
      const collection = data.slice(indexoffirstpage,indexoflastpage)
  ...
  return (
    ...
    {data.map((items)=>(
      ...
      <div className={snap.contents}>
        ...
        <p className={snap.s2}>{items.product_name}</p>
        <div className={snap.s3}>
              <del className={snap.s5}>{items.product_price}</del>
               <p className={snap.s6}>{items.product_offer}</p>
          {items.fixedprice ? <p className={snap.s4}>{items.fixedprice}</p>:""}
        </div>
        <p className={snap.buttons}  onClick={() => addToCart(items)}>Add to cart</p>
      </div>
